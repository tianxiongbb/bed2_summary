...
if __name__ == '__main__':
    ...
    if cpu >= 1:
        ...
        pool = Pool(cpu)
        pool.apply_async(run, args=(dict_bed2_watson_uniq, chrom_size, dict_chrom, prefix + ".watson.uniq"))
        pool.apply_async(run, args=(dict_bed2_watson_all, chrom_size, dict_chrom, prefix + ".watson.all"))
        pool.apply_async(run, args=(dict_bed2_crick_uniq, chrom_size, dict_chrom, prefix + ".crick.uniq"))
        pool.apply_async(run, args=(dict_bed2_crick_all, chrom_size, dict_chrom, prefix + ".crick.all"))
        pool.close()
        pool.join()
